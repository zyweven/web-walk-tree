...
## 用户偏好与约束
- 展示布局：径向（辐射状）树为主展示。
- 激活方式：手动激活或浏览器启动即激活，支持随时暂停。
- 树根规则：无来源页面即为树根；同一会话/一天内可能存在多个树根。
- 会话定义：浏览器打开到关闭视为一次记录；过去历史可查看。
- 导出格式：先仅支持 JSON。
- 隐私：数据只保存在本机，不上传；支持域名黑名单。
...
- 事件源：
  ...
  - `tabs.onActivated` 辅助识别当前活跃页面；用于面包屑更新。
...
## 数据模型
- Node：`id`、`url`、`title`、`faviconUrl`、`firstSeenAt`、`lastSeenAt`、`visitCount`、`sessionId`、`isRoot`
- Edge：`id`、`fromNodeId`、`toNodeId`、`type(click|typed|keyword|redirect|back|forward)`、`createdAt`、`anchorText`、`domPath`、`openInNewTab`、`confidence`
- Session：`id`、`date`、`startedAt`、`endedAt`、`rootNodeIds[]`、`notes`、`isPaused`
...
```
{
  ...
  "nodes": [
    ...
    {"id":"n2","url":"https://example.com/blog","title":"Blog","faviconUrl":"...","firstSeenAt":1234567900,"lastSeenAt":1234567900,"visitCount":1,"sessionId":"2025-11-15-1","isRoot":false}
  ],
  "edges": [
...
## 里程碑与验收
- 阶段 1：自动会话、树根识别、节点/边存储、列表查看
- 阶段 2：点击意图与新标签准确关联，重定向合并
- 阶段 3：侧边面板径向树与面包屑（主交互完成）
- 阶段 4：历史会话与 JSON 导出、过滤黑名单
- 阶段 5：SPA 支持、性能优化与大图虚拟化
...
## 下一步
- 依据本文档按里程碑推进实现；侧边面板为主交互，先完成最小闭环再扩展到 SPA 与性能优化。